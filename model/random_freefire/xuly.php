...
if($row['giatien'] > $user['money']) {
    ...
    mysqli_query($kun->connect_db(), "INSERT INTO `user_history_system` (`username`, `action`, `action_name`, `sotien`, `mota`, `time`, `hisnick`) VALUES ('".$user['username']."', 'Mua Acc Random', '".$thread['id']."', '-".number_format($row['giatien'])."đ', 'Mua Acc #".$id." Loại ".$row['name']."', '".time()."', '3')");
